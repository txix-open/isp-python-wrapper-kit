...
* Передаёт ему `json` (`config.json`) конфигурацию через файл и env-переменные:
	* `BINDING_ADDRESS` - адрес, на котором запускается сервис 
	* `CONFIG_FILE` - путь до конфигурационного файла
	* `REQUIRED_MODULES` - список сервисов, для которых ожидаются адреса, разделённый `;`
