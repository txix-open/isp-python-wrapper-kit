...
4) Restarting (перезапуск при получении нового конфига) - выключение дочернего процесса и повторный запуск
5) Stopping (остановка) - передача дочернему процессу сигнала `SIGTERM` с таймаутом 5s, если за это время процесс не останавливается, вызывается `kill` дочернего процесса
6) Stopped (завершён)
